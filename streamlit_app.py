...
st.set_page_config(page_title="Vantaje Algoritmo v2.8.6", layout="wide")

st.title("🎾 Vantaje Algoritmo v2.8.6")
st.markdown("### Dashboard de Análisis Predictivo & Respaldo en Redes")
...
with col_m2:
    # Ajuste: Nombre, Rank y Cuota para Jugador 1
...
with col_m3:
    # Ajuste: Nombre, Rank y Cuota para Jugador 2
...
def cargar_jugador_full(nombre):
    ...
    for i in range(3):
        with st.expander(f"Partido Reciente {i+1}", expanded=(i==0)):
            c1, c1_f, c2, c3 = st.columns([1.5, 1, 1, 1])
            ...
            partidos.append({"res": res, "p1":p1, "d1":d1})
    return partidos
...
# --- SECCIÓN 3: VEREDICTO Y POST ---
if st.button("EJECUTAR ANÁLISIS VANTAGE"):
    avg_j1_p1 = sum([p['p1'] for p in data_j1]) / 3
    avg_j1_d1 = sum([p['d1'] for p in data_j1]) / 3
    avg_j2_p1 = sum([p['p1'] for p in data_j2]) / 3
    avg_j2_d1 = sum([p['d1'] for p in data_j2]) / 3
    ...
    # Lógica de semáforo para el post basado en el análisis
    ...
    st.subheader("📱 Generador de Post (Copy-Paste)")
...
📊 Dato Vantaje: {ganador_proy} llega con {max(avg_j1_p1, avg_j2_p1):.1f}% de G1S. El diferencial de eficiencia es clave.

#TennisBets #ATP #VantajeAlgoritmo"""
    
    st.text_area("Copia esto para Twitter:", texto_post, height=200)
    
    st.divider()
    st.metric("Poder Combinado", f"{max(puntos_j1, puntos_j2):.1f}", f"{abs(puntos_j1 - puntos_j2):.1f} vs rival")
